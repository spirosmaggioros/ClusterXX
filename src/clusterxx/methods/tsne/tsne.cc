...
void clusterxx::TSNE::__fit(const std::vector<std::vector<double>> &X) {
    ...
    std::vector<std::vector<double>> p_ji =
        compute_pairwise_affinities(X, __perplexity);
    ...
    for (int i = 0; i < __max_iter; i++) {
        ...
        std::vector<std::vector<double>> q_ij =
            compute_low_dim_affinities(solution);
...
clusterxx::TSNE::fit_transform(const std::vector<std::vector<double>> X) {
    __fit(X);
    return __features;
}

std::pair<int, int> clusterxx::TSNE::get_shape() { return __shape; }
