        ...
        int max_iter=100)
{
    ...
    while (max_iter--) {
        ...
        double entropy = std::log(sum_exp) + sum_exp_tot / (sum_exp * 2 * sigma * sigma);
...
std::vector<std::vector<double> > compute_pairwise_affinities(
        std::vector<std::vector<double> > features,
        double perplexity)
{
    std::vector<std::vector<double> > p_ji(features.size(), std::vector<double>(features.size()));
    for (size_t i = 0; i<features.size(); i++) {
        ...
        for (size_t j = 0; j<features.size(); j++) {
            if (i == j) [[unlikely]] {
                ...
                p_ji[i][j] = std::exp(-1.0 * distances[dist_idx] / (2.0 * sigma * sigma));
                sum += p_ji[i][j];
                dist_idx++;
            }
        }
...
std::vector<std::vector<double> > compute_low_dim_affinities(
        const std::vector<std::vector<double> > Y) 
{
    std::vector<std::vector<double> > q_ij(Y.size(), std::vector<double>(Y.size()));
    std::vector<double> distances;
...
std::vector<std::vector<double> > kullback_leibler_gradient(
        ...
        std::vector<std::vector<double> > low_dim_features) 
{
    ...
    std::vector<std::vector<double> > gradient(pairwise_affinities.size(), std::vector<double>(low_dim_features[0].size()));

    for (size_t i = 0; i<pairwise_affinities.size(); i++) {
        for (size_t j = 0; j<pairwise_affinities[0].size(); j++) {
            if (i != j) {
                double factor = 4.0 * (pairwise_affinities[i][j] - low_dim_affinities[i][j]) / (1.0 + vector_diff(low_dim_features[i], low_dim_features[j]));

                for (size_t k = 0; k<low_dim_features[0].size(); k++) {
                    gradient[i][k] += factor * (low_dim_features[i][k] - low_dim_features[j][k]);
                }
            }
        }
    }
