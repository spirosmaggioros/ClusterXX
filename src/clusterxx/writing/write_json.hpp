...
namespace clusterxx {
    ...
    std::string save_to_tmp_json_clustering(const arma::mat &X, const std::vector<int> &labels) {
        nlohmann::json j;
        j["features"] = X;
        j["labels"] = labels;
        std::filesystem::path tmp_filepath = 
            std::filesystem::temp_directory_path() / std::filesystem::path("clusterxx-%%%%-%%%%.json");
