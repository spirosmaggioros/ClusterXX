...
TEST_CASE("Testing vp-tree query radius with default distance metric", "[vp-tree]") {
    ...
    clusterxx::vp_tree vp_tree = clusterxx::vp_tree(X);
    auto [indices, dists] = vp_tree.query_radius(X.row(0).t(), 0.3);

    REQUIRE(indices.size() == 3);
    REQUIRE(std::find(indices.begin(), indices.end(), 0) != indices.end());
    REQUIRE(std::find(indices.begin(), indices.end(), 3) != indices.end());
    ...
    X = {{0.5488135 , 0.71518937, 0.60276338},
       ...
       {0.11827443, 0.63992102, 0.14335329},
       {0.94466892, 0.52184832, 0.41466194}};
    clusterxx::vp_tree vp_tree_2 = clusterxx::vp_tree(X);
    auto [indices_2, dists_2] = vp_tree.query(X.row(0).t(), 3);
    ...
    clusterxx::vp_tree vp_tree3 = clusterxx::vp_tree(X);
    auto [inds_3, dists_3] = vp_tree3.query_radius(X.row(0).t(), 0.3);
...
TEST_CASE("Testing vp-tree query radius with manhattan distances", "[vp-tree]") {
    arma::mat X = {{0.5488135 , 0.71518937, 0.60276338},
       {0.54488318, 0.4236548 , 0.64589411},
       {0.43758721, 0.891773  , 0.96366276},
       {0.38344152, 0.79172504, 0.52889492},
       ...
       {0.11827443, 0.63992102, 0.14335329},
       {0.94466892, 0.52184832, 0.41466194}};
    clusterxx::vp_tree<clusterxx::metrics::manhattan_distance> vp_tree =
        clusterxx::vp_tree<clusterxx::metrics::manhattan_distance>(X);
    auto [inds, dists] = vp_tree.query(X.row(0).t(), 3);
    ...
    clusterxx::vp_tree<clusterxx::metrics::manhattan_distance> vp_tree_2 =
        clusterxx::vp_tree<clusterxx::metrics::manhattan_distance>(X);

    auto [inds_2, dists_2] = vp_tree_2.query(X.row(0).t(), 5);
